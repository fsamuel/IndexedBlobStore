...
// Popualte a B+ tree with 100 elements in random order. Verify that the
// elements are in the tree. Delete the elements also in random order. Verify
// that the elements are no longer in the tree.
...
TEST_F(BPlusTreeTest, TransactionInsertion) {
  ...
  // Search for 75 inside the transaction to find it, see it's the correct value
  // and outside to verify that it's not there.
...
// Insert 50 elements into the B+ tree. Create a transaction, insert a few more
// through the transaction. Verify that the first 50  appear outside the
// transaction and all 55 appear inside the transaction. Abort the transaction
// and verify that the first 50 elements are still in the tree and the last 5
// are not.
TEST_F(BPlusTreeTest, InsertTransactionAbort) {
  ...
  // Verify that only the first 50 elements are in the tree outside the
  // transaction.
  for (int i = 0; i < 55; i++) {
    auto it = tree.Search(i);
    auto value_ptr = it.GetValue();
...
TEST_F(BPlusTreeTest, StringInsertion) {
  ...
  for (int i = 0; i < 100; i++) {
    ...
    std::string value_output =
        value_ptr == nullptr ? std::string() : *value_ptr;
    EXPECT_NE(value_ptr, nullptr);
