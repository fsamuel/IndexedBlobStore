...
// Use SerializeTraits to serialize another BlobStoreObject to a blob and then
// deserialize it again.
TEST_F(BlobStoreTest, SerializeTraitsBlobStoreObject) {
  BlobStore store(TestMemoryBufferFactory::Get(), "MetadataBuffer", 4096,
                  std::move(*dataBuffer));

  BlobStoreObject<const int> input =
      std::move(store.New<int>(1337)).Downgrade();
  BlobStoreObject<char[]> ptr = store.Serialize(input);

  BlobStoreObject<const int> output;
  ptr.Deserialize(&output);
