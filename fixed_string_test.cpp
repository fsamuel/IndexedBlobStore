...
TEST(FixedStringTest, Constructor) {
  ...
  EXPECT_EQ(fs->hash,
            std::hash<std::string>{}(test_string));  // Check if hash matches
  EXPECT_EQ(std::memcmp(fs->data, test_string.data(), fs->size),
            0);  // Check if data matches
}
...
TEST(FixedStringTest, ConversionToString) {
  ...
  std::string converted_string =
      static_cast<std::string>(*fs);  // Convert FixedString to std::string

  EXPECT_EQ(
      test_string,
      converted_string);  // The original and converted string should match
}
...
TEST(FixedStringTest, ConversionToStringSlice) {
  ...
  StringSlice converted_slice =
      static_cast<StringSlice>(*fs);  // Convert FixedString to StringSlice

  EXPECT_EQ(converted_slice.data(),
            fs->data);  // The data pointers should match
...
TEST(FixedStringTest, StreamInsertionOperator) {
  ...
  EXPECT_EQ(ss.str(), test_string);  // The string inserted into the
                                     // stringstream should match the original
}