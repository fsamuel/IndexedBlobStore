...
struct TransactionHelper<KeyType, ValueType, Order, BaseNode<Order>> {
	...
	static BlobStoreObject<BaseNode> GetMutable(Transaction* txn, BlobStoreObject<const BaseNode> node) {
		if (node->is_leaf()) {
			return TransactionHelper<KeyType, ValueType, Order, LeafNode>::GetMutable(txn, std::move(node).To<LeafNode>()).To<BaseNode>();
		}
		return TransactionHelper<KeyType, ValueType, Order, InternalNode>::GetMutable(txn, std::move(node).To<InternalNode>()).To<BaseNode>();
	}
};
...
struct TransactionHelper<KeyType, ValueType, Order, InternalNode<Order>> {
	...
	static BlobStoreObject<InternalNode> New(Transaction* txn, Args&&... args) {
		BlobStoreObject<InternalNode> node = txn->blob_store_->New<InternalNode>(std::forward<Args>(args)...);
...
public:
	...
	BlobStoreObject<T> New(Args&&... args) {
		return TransactionHelper<KeyType, ValueType, Order, T>::New(this, std::forward<Args>(args)...);
	}

	template<typename T>
	BlobStoreObject<typename std::remove_const<T>::type> GetMutable(BlobStoreObject<typename std::add_const<T>::type> node) {
		return TransactionHelper<KeyType, ValueType, Order, typename std::remove_const<T>::type>::GetMutable(this, std::move(node));
	}
...
private:
	...
	template<typename KeyType, typename ValueType, std::size_t Order, typename T> friend struct TransactionHelper;
};
