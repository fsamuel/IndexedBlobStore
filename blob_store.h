...
class BlobStoreObject {
 public:
  ...
  BlobStore* GetBlobStore() const { return control_block_->store_; }
...
class BlobStore {
 public:
  ...
  typename std::enable_if<
      ...
      BlobStoreObject<typename std::remove_const<T>::type>>::type
  Clone(size_t index) {
    ...
    return BlobStoreObject<typename std::remove_const<T>::type>(this,
                                                                clone_index);
  }

  bool CompareAndSwap(size_t index,
 ...
 private:
  ...
  bool AcquireWriteLock(size_t index) {
    ...
    while (true) {
      ...
      if (metadata->lock_state.compare_exchange_weak(
              expected, WRITE_LOCK_FLAG, std::memory_order_acquire)) {
        break;
      }
      SpinWait();
    }
  ...
  void UpgradeReadLock(size_t index) {
    ...
    while (true) {
      ...
      if (metadata->lock_state.compare_exchange_weak(expected,
                                                     WRITE_LOCK_FLAG)) {
        break;
      }
      SpinWait();
