...
public:
    explicit SharedMemoryVector(const allocator_type& alloc) : m_allocator(alloc), m_size(0), m_data(-1) {}

    explicit SharedMemoryVector(size_type count, const T& value = T(), const allocator_type& alloc = allocator_type())
        : m_allocator(alloc), m_size(count), m_data(m_allocator.allocate(count * sizeof(T)))
    {
    ...
    SharedMemoryVector(const SharedMemoryVector& other)
        : m_allocator(other.m_allocator), m_size(other.m_size), m_data(m_allocator.allocate(m_size))
    {
        for (size_type i = 0; i < m_size; ++i) {
    ...
    SharedMemoryVector(SharedMemoryVector&& other)
        : m_allocator(std::move(other.m_allocator)), m_size(other.m_size), m_data(other.m_data)
    {
        other.m_size = 0;
    ...
    iterator begin() noexcept {
        return m_allocator.ToPtr<T>(m_data);
    }
    ...
    iterator end() noexcept {
        return m_allocator.ToPtr<T>(m_data + m_size * sizeof(T));
    }
    ...
    size_type capacity() const noexcept {
        return m_allocator.capacity(m_data);
    }
    ...
    void reserve(size_type new_cap) {
        ...
        for (size_type i = 0; i < m_size; ++i) {
            m_allocator.construct(new_data + i, std::move_if_noexcept(m_allocator.ToPtr<T>(m_data)[i]));
            m_allocator.destroy(m_allocator.ToPtr<T>(m_data) + i);
        }
        m_allocator.deallocate(m_data);
        m_data = m_allocator.ToOffset(new_data);
    }
    ...
    reference front() {
        return m_allocator.ToPtr<T>(m_data)[0];
    }
    ...
    reference back() {
        return m_allocator.ToPtr<T>(m_data)[m_size - 1];
    }
    ...
    void emplace_back(Args&&... args) {
        ...
        m_allocator.construct(m_allocator.ToPtr<T>(m_data + m_size * sizeof(T)), std::forward<Args>(args)...);
        ++m_size;
    }
    ...
    void pop_back() {
        ...
        m_allocator.destroy(m_allocator.ToPtr<T>(m_data) + m_size - 1);
        --m_size;
    }
    ...
    void resize(size_type count, const value_type& value = value_type()) {
        if (count > m_size) {
            ...
            for (size_type i = m_size; i < count; ++i) {
                m_allocator.construct(m_allocator.ToPtr<T>(m_data) + i, value);
            }
        }
