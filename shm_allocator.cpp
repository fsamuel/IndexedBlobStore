...
ShmAllocator::ShmAllocator(ShmAllocator&& other)
    : chunk_manager_(std::move(other.chunk_manager_)), operations_(std::move(other.operations_)) {}
...
uint8_t* ShmAllocator::Allocate(std::size_t bytes_requested) {
    ...
    while (true) {
        ...
        // No block of sufficient size was found. We need to request a new chunk, add
        // it to the free list, and try again.
...
uint8_t* ShmAllocator::NewAllocatedNode(uint8_t* buffer, std::size_t index, std::size_t size) {
...
typename ShmAllocator::Node* ShmAllocator::SearchBySize(
    std::size_t size,
    std::size_t index,
    Node** left_node) {
        ...
        while (true) {
            ...
            if (skipped_nodes.insert(current_node_next_index).second) {
                operations.emplace_back(
                    OperationType::Search, *current_node);
            }
            else if (!printed) {
                operations.emplace_back(
                    OperationType::Search, *current_node);
