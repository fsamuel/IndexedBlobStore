...
class StringSlice {
 public:
  ...
  const char* data() const { return str_ + offset_; }

  // Returns a new StringSlice representing a slice of this StringSlice.
  // The new slice starts at the given offset within this slice and has the
  // given size. If the given offset and size would extend beyond the end of
  // this slice, the size of the new slice is reduced to fit within this slice.
  ...
  bool operator>(const char* str) const { return str < *this; }
...
struct hash<StringSlice> {
  size_t operator()(const StringSlice& slice) const {
    ...
    for (size_t i = 0; i < slice.size(); ++i) {
      ...
      // It may not be suitable for all purposes, but it's a good starting
      // point.
