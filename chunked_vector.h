...
public:
    ...
    ChunkedVector(ChunkedVector&& other):
        name_prefix_(std::move(other.name_prefix_)), size_(std::move(other.size_)), chunks_(std::move(other.chunks_)), chunk_size_(std::move(other.chunk_size_)) {}
...
private:
    ...
    // Pointer to the size of the vector. This is stored at the start of the first chunk.
...
void ChunkedVector<T>::resize(std::size_t new_size) {
    ...
    } while (!std::atomic_compare_exchange_strong(size_, &expected_size, new_size));
}
