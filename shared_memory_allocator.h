...
class SharedMemoryAllocator {
 ...
 private:
  ...
  void InitializeAllocatorStateIfNecessary() {
    ...
    if (state_header_ptr->magic_number != 0x12345678) {
      ...
      T* buffer = NewAllocatedNode(
          data, 0, chunk_manager_.capacity() - sizeof(AllocatorStateHeader));
  ...
  T* NewAllocatedNode(uint8_t* buffer,
                      std::size_t chunk_index,
                      std::size_t size) {
...
T* SharedMemoryAllocator<T>::Allocate(std::size_t bytes_requested) {
  ...
  while (true) {
    ...
    if (chunk_manager_.get_or_create_chunk(last_num_chunks, &new_chunk_data,
                                           &new_chunk_size) > 0) {
      T* buffer =
          NewAllocatedNode(new_chunk_data, last_num_chunks, new_chunk_size);
      GetNode(buffer)->version.store(1);
      Deallocate(buffer);
...
bool SharedMemoryAllocator<T>::Deallocate(T* ptr) {
  ...
  while (true) {
    ...
    // Set the next pointer of the new free node to the current head of the free
    // list
    ...
    if (state()->free_list_index.compare_exchange_weak(free_list_index,
                                                       node_header_index)) {
...
template <typename T>
std::uint64_t SharedMemoryAllocator<T>::ToIndexImpl(Node* ptr,
                                                    std::true_type) const {
  ...
  return chunk_manager_.encode_index(chunk_index,
...
template <typename T>
template <typename U>
std::uint64_t SharedMemoryAllocator<T>::ToIndexImpl(U* ptr,
...
T* SharedMemoryAllocator<T>::AllocateFromFreeNode(std::size_t bytes_needed) {
  ...
  while (current_free_node != nullptr) {
    ...
    // If we've found a block greater than the size we need (and can accommodate
    // a head a second block, split it into two blocks. The first piece will be
    // returned to the free list, and the second piece will be returned to the
    // caller.
    if (free_node_size > bytes_needed + sizeof(Node)) {
      std::size_t new_free_node_size = free_node_size - bytes_needed;
      if (!current_free_node->size.compare_exchange_weak(free_node_size,
                                                         new_free_node_size)) {
        // The block was resized by another thread, go back to the free list and
        // try again.
    ...
    // The block is large enough to use for allocation but NOT large enough to
    // split.
    if (free_node_size >= bytes_needed) {
      // Found a block that is large enough, remove it from the free list and
      // return a pointer to its data.
      if (prev_free_node != nullptr) {
        std::size_t current_free_node_index = ToIndex(current_free_node);
        if (current_free_node_index == InvalidIndex ||
            !prev_free_node->next_index.compare_exchange_weak(
                current_free_node_index,
                current_free_node->next_index.load()) ||
            !prev_free_node->is_free() ||
            prev_free_node->version.load() != prev_free_node_version) {
          ...
          // The block was removed from the free list by another thread, go back
          // to the beginning of the free list and try again.
          prev_free_node = nullptr;
          prev_free_node_version = 0;
          current_free_node = FirstFreeNode();
          continue;
        ...
        // The block we found is the first block in the free list, update the
        // free list index. Again this needs to be done atomically or we need to
        // move back to the beginning of the free list and try again. This
        // design likely results in a lot of fragmentation. We should consider
        // using a free list that is sorted by size
        std::size_t current_free_node_index = ToIndex(current_free_node);
        if (current_free_node_index == InvalidIndex ||
            !state()->free_list_index.compare_exchange_weak(
                current_free_node_index,
                current_free_node->next_index.load())) {
          // The block was removed from the free list by another thread, go back
          // to the beginning of the free list and try again.
      ...
      return NewAllocatedNode(reinterpret_cast<uint8_t*>(current_free_node),
                              chunk_index,
                              std::max(bytes_needed, free_node_size));
    }
