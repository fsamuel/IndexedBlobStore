...
thread_local std::map<const AllocationLogger*, std::set<std::size_t>> AllocationLogger::skipped_nodes_;
...
AllocationLogger::Operation::Operation(OperationType type, const ShmAllocator::Node& node) :
...
void AllocationLogger::PrintOperation(const Operation& operation) const {
    std::cout << "ThreadId(" << operation.thread_id << "): " <<
        OperationTypeToString(operation.type) << "(" << ChunkManager::chunk_index(operation.index)
...
void AllocationLogger::PrintIndexHistory(std::size_t index) const {
    ...
    for (int i = std::max<int>(0, op_size - 2000); i < op_size; ++i) {
        ...
        std::size_t op_offset_in_chunk = ChunkManager::offset_in_chunk(operation.index);
