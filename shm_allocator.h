...
public:
	...
	static constexpr std::size_t InvalidIndex = std::numeric_limits<std::size_t>::max() >> 1;
...
private:
	...
	void InitializeAllocatorStateIfNecessary() {
		...
		if (state_header_ptr->magic_number != 0x12345678) {
			...
			T* buffer = NewAllocatedNode(data, chunk_manager_.encode_index(0, sizeof(AllocatorStateHeader)), chunk_manager_.capacity() - sizeof(AllocatorStateHeader));
	...
	bool is_marked_reference(size_t value) {
		size_t mask = (size_t)1 << (sizeof(size_t) * 8 - 1); // shift 1 to the leftmost bit
		return value & mask; // bitwise AND, if the highest bit is set, the result is nonzero
	}
...
T* ShmAllocator<T>::Allocate(std::size_t bytes_requested) {
	...
	while (true) {
		...
		if (chunk_manager_.get_or_create_chunk(last_num_chunks, &new_chunk_data, &new_chunk_size) > 0) {
			T* buffer = NewAllocatedNode(new_chunk_data, chunk_manager_.encode_index(last_num_chunks, 0), new_chunk_size);
			GetNode(buffer)->version.store(1);
			Deallocate(buffer);
...
bool ShmAllocator<T>::Deallocate(T* ptr) {
	...
	do {
		...
		if (left_node == nullptr) {
			if (state()->free_list_index.compare_exchange_weak(right_node_index, node->index)) {
				return true;
			}
		}
		else {
			if (left_node->next_index.compare_exchange_weak(right_node_index, node->index)) {
				return true;
			}
		}
	} while (true); /*B3*/
...
T* ShmAllocator<T>::AllocateFromFreeList(std::size_t bytes_needed) {
	...
	do {
		...
		if (!is_marked_reference(right_node_next_index)) {
			if ((right_node->next_index.compare_exchange_weak(right_node_next_index, get_marked_reference(right_node_next_index)) )) {
				break;
			}
		}
	} while (true); /*B4*/
	...
	if (left_node == nullptr) {
		...
		if (!left_node->next_index.compare_exchange_weak(right_node_index, right_node_next_index)) {
			SearchBySize(right_node->size, &left_node);
		}
	}
...
template<typename T>
typename ShmAllocator<T>::Node* ShmAllocator<T>::SearchBySize(std::size_t size, Node** left_node) {
	...
	do {
		...
		do {
			...
			current_node = ToPtr<Node>(get_unmarked_reference(current_node_next_index));
			if (current_node == nullptr) {
				break;
			}
			current_node_next_index = current_node->next_index.load();
		} while (is_marked_reference(current_node_next_index) || (current_node->size < size));
		...
		if (left_node_next_index == right_node_index) {
			if ((right_node != nullptr) && is_marked_reference(right_node->next_index.load())) {
		...
		//std::size_t right_node_index = right_node == nullptr ? InvalidIndex : right_node->index;
		if ((*left_node) != nullptr) {
			if ((*left_node)->next_index.compare_exchange_weak(left_node_next_index, right_node_index)) {
				if (right_node != nullptr && is_marked_reference(right_node->next_index.load())) {
					goto search_again;
				}
				else {
			...
			if (state()->free_list_index.compare_exchange_weak(left_node_next_index, right_node_index)) {
				if (right_node != nullptr && is_marked_reference(right_node->next_index.load())) {
					goto search_again;
				}
				else {
